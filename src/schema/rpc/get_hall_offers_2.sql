CREATE OR REPLACE FUNCTION public.get_hall_offers(page_limit integer DEFAULT 20, page_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, title text, price numeric, images text[], seller jsonb, offer_type text, created_at text, views_count integer, turnaround_days integer, total_count bigint, has_more boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Return offers from the same hall with pagination
  RETURN QUERY
  SELECT 
    o.id,
    o.title,
